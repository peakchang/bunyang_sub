<script>
    ...
    import SortableImgOnly from "$lib/components/SortableImgOnly.svelte";
    ...
    let tempSaveImgs = $state([]); // 임시저장 이미지 리스트, 새로고침 / 뒤로가기시 싹 삭제됨
    let modifyVal = $state(data.modifyVal);
    let allData = $state(data.modifyVal);
    let mainImgs = $state([]);
    let bannerImgs = $state([]);
    let getId = $state("");
    let imgActType = $state("");

    // 사이트 복사 기능!!
    let siteCopyAreaShow = $state(false);
    let copyDomain = $state("");

    const gogo = "a,b,c,d,e,f,g,h,i,j";

    let testval = $state("");

    if (modifyVal.ld_main_img) {
        mainImgs = modifyVal.ld_main_img.split(",");
    }

    if (modifyVal.ld_banner_img) {
        bannerImgs = modifyVal.ld_banner_img.split(",");
    }
